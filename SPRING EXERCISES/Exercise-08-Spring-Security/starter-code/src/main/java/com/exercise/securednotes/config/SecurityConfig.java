...
public class SecurityConfig {
    ...
    //         Method signature: public SecurityFilterChain filterChain(HttpSecurity http) throws Exception
    //         Configure it to:
    ...
    //           - Return http.build()
    //
    //         HTTP Basic means: clients send "Authorization: Basic base64(username:password)" header

    // TODO 3: Create a UserDetailsService bean (InMemoryUserDetailsManager).
    //         This defines who can log in. Create two users:
    //           User 1: username="user",  password=passwordEncoder().encode("Password1!"), roles="USER"
    //           User 2: username="admin", password=passwordEncoder().encode("Admin123!"), roles="ADMIN"
