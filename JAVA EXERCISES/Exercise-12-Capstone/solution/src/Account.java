...
public abstract class Account {
    ...
    Account(String accountNumber, String ownerName, double initialBalance) throws BankException {
        if (initialBalance < 0)
            throw new BankException.InvalidAmountException("Initial balance cannot be negative");
    ...
    public void deposit(double amount) throws BankException {
        if (amount <= 0)
            throw new BankException.InvalidAmountException("Deposit amount must be positive");
        balance += amount;
