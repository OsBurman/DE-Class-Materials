...
public class Library {
    ...
    public void addMember(Member m) {
        if (memberCount < members.length) members[memberCount++] = m;
    }
    ...
    private Member findMember(int memberId) {
        for (int i = 0; i < memberCount; i++)
            if (members[i].getId() == memberId) return members[i];
        return null;
    }

    public Member getMember(int memberId) { return findMember(memberId); }
    ...
    public void checkout(int memberId, int bookId) {
        ...
        if (!book.isAvailable()) { System.out.println("Book is already checked out."); return; }
    ...
    public void returnBook(int memberId, int bookId) {
        Book   book   = findBook(bookId);
        Member member = findMember(memberId);
        if (book == null || member == null) { System.out.println("Invalid id."); return; }
