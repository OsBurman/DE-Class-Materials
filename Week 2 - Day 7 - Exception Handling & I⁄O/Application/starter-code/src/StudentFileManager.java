 ...
 * Complete all TODOs using BufferedReader, BufferedWriter, and try-with-resources.
 */
public class StudentFileManager {

    // TODO Task 3: readStudents(String filePath)
    // - Use try-with-resources: try (BufferedReader br = new BufferedReader(new FileReader(filePath)))
    // - Skip the header line
    // - For each data line: split by ",", parse gpa
    // - If gpa < 0.0 or > 4.0: throw new InvalidGradeException(...)
    // - If gpa can't be parsed: catch NumberFormatException and print warning, skip row
    ...
    // - Use try-with-resources: try (BufferedWriter bw = new BufferedWriter(new FileWriter(filePath)))
